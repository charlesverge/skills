...
- Avoid adding parameters with a default value or a type or None. This causes bugs to occur as it hides type check errors. Is a very strong case to use parameter\_name: type | None = None then stop and request permission.
...
1. **Composition**: Create a wrapper or aggregator class that holds instances of the target class and delegates to their public API.
1. **Shared instance**: Use a single shared instance that all callers reference.
