...
2. Compute decisions once from existing state and new input.
3. Write one mutation once per execution path.
4. Return decisions once in a `DecisionResult` object.
...
2. `Decide`: Compute outcomes from fetched state and event input.
3. `Record modification`: Build the exact mutation payload.
4. `Update (DB record/api)`: Persist one write and return `DecisionResult`.
...
2. `decide_*` method for `Decide`
3. `record_modification_*` method for `Record modification`
4. `update_*` method for `Update (DB record/api)`
...
2. Ask whether failures should retry the entire operation or fail fast without retry.
3. Ask which log level should be used for failure cases (`error`, `warning`, or `info`) and whether different exception types need different levels.
4. Ask whether failed operations should be queued for delayed retry, including the trigger conditions for queueing.
5. Ask whether delayed retry should have limits (max attempts, backoff strategy, or cutoff time) or use existing application defaults.
...
1. read/update call,
2. then a second read of the same entity,
3. only to recover outcomes that the update method should have returned.
...
2. Keep only the first read used for decisioning.
3. Move decision calculations into the update method.
4. Return a complete `DecisionResult` from that method.
5. Replace caller follow-up reads with `DecisionResult` fields.
6. Verify one read + one write per execution path in tests and code review.
